  ...
  | hit s0h =>
    ...
    | raw =>
      ...
      have hx : sz q_v0 < sz (p (p q_x (p q_v1 q_x)) (p q_v0 q_v0)) := by
        ...
        have ev : sz H0 = sz (p (p q_x (p q_v1 q_x)) (p q_v0 q_v0)) := congrArg sz (p1)
...
theorem nr1 (x v0 v1 : CM)
 :
  ...
  | raw =>
    ...
    have cyc : q_v0 = (p (p q_x (p q_v1 q_x)) (p q_v0 q_v0)) := (let peq0 : v0 = q_v0 := e0; let peq1 : v0 = (p (p q_x (p q_v1 q_x)) (p q_v0 q_v0)) := e1; let pst0 : q_v0 = v0 := Eq.symm (peq0); let pst1 : q_v0 = (p (p q_x (p q_v1 q_x)) (p q_v0 q_v0)) := Eq.trans (pst0) (peq1); pst1)
...
theorem nr2 (x v0 v1 H0 : CM)
  ...
  cases s0 with
  | raw =>
    have he : q_H0 = q_x := (let peq0 : (p x (p v1 x)) = q_v0 := ha; let peq1 : v0 = (p q_x q_H0) := congrArg (fun q => (L q)) (hb); let peq2 : v0 = (p q_v0 q_v0) := congrArg (fun q => (R q)) (hb); let pst0 : (p q_x q_H0) = v0 := Eq.symm (peq1); let pst1 : (p q_x q_H0) = (p q_v0 q_v0) := Eq.trans (pst0) (peq2); let pst2 : q_v0 = (p x (p v1 x)) := Eq.symm (peq0); let pst3 : (p q_v0 q_v0) = (p (p x (p v1 x)) q_v0) := congrArg (fun q => p q q_v0) (pst2); let pst4 : (p (p x (p v1 x)) q_v0) = (p (p x (p v1 x)) (p x (p v1 x))) := congrArg (fun q => p (p x (p v1 x)) q) (pst2); let pst5 : (p q_v0 q_v0) = (p (p x (p v1 x)) (p x (p v1 x))) := Eq.trans (pst3) (pst4); let pst6 : (p q_x q_H0) = (p (p x (p v1 x)) (p x (p v1 x))) := Eq.trans (pst1) (pst5); let pst7 : q_H0 = (p x (p v1 x)) := congrArg (fun q => R q) (pst6); let pst8 : q_x = (p x (p v1 x)) := congrArg (fun q => L q) (pst6); let pst9 : (p x (p v1 x)) = q_x := Eq.symm (pst8); let pst10 : q_H0 = q_x := Eq.trans (pst7) (pst9); pst10)
