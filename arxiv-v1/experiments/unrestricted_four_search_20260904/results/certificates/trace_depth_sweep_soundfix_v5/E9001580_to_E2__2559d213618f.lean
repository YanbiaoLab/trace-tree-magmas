...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_v2 q_H0 q_H1 q_H2 : CM, Step q_v1 q_v2 q_H0 ∧ Step q_x q_H0 q_H1 ∧ Step q_v0 (p q_H1 q_x) q_H2 ∧ a = (p q_H2 q_v0) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
...
theorem source_holds (x v0 v1 v2 : CM) :
    x = (eval (eval (eval v0 (eval (eval x (eval v1 v2)) x)) v0) v0) := by
  let H0 := eval v1 v2
  have e0a : v1 = v1 := by
    change v1 = v1
    rfl
  ...
  let H2 := eval v0 (eval (eval x (eval v1 v2)) x)
  have e2a : v0 = v0 := by
    change v0 = v0
    rfl
  have e2b : (eval (eval x (eval v1 v2)) x) = (p H1 x) := by
    change (eval H1 x) = (p H1 x)
    exact (eval_raw (nr0 x v0 v1 v2 H1 s1))
  have s2 : Step v0 (p H1 x) H2 := by
    rw [← e2a, ← e2b]
    exact eval_step v0 (eval (eval x (eval v1 v2)) x)
  change x = (eval (eval H2 v0) v0)
  have rawEq : (eval (eval H2 v0) v0) = (eval (p H2 v0) v0) := congrArg (fun q => (eval q v0)) (eval_raw (nr1 x v0 v1 v2 H2 s2))
