...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_v2 q_H0 q_H1 q_H2 q_H3 : CM, Step q_x q_v2 q_H0 ∧ Step q_v1 q_H0 q_H1 ∧ Step q_v0 (p q_H1 q_v1) q_H2 ∧ Step q_x q_v0 q_H3 ∧ a = (p q_H2 (p q_H3 q_x)) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          change v = (p q_v0 (p (p q_v1 (p q_x q_v2)) q_v1)) at e0
        ...
        | hit qs3h =>
          ...
          change v = (p q_v0 (p (p q_v1 (p q_x q_v2)) q_v1)) at p0
      ...
      | raw =>
        ...
        | raw =>
          ...
          | raw =>
            ...
            change b = (p (p q_v1 (p q_x q_v2)) q_v1) at e1
            ...
            exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
          | hit qs3h =>
            have cyc : q_x = (p (p q_v1 (p q_x q_v2)) q_v1) := (let peq1 : b = (p (p q_v1 (p q_x q_v2)) q_v1) := congrArg (fun q => (R (L q))) (ha); let peq4 : b = q_x := ho; let pst0 : (p (p q_v1 (p q_x q_v2)) q_v1) = b := Eq.symm (peq1); let pst1 : (p (p q_v1 (p q_x q_v2)) q_v1) = q_x := Eq.trans (pst0) (peq4); let pst2 : q_x = (p (p q_v1 (p q_x q_v2)) q_v1) := Eq.symm (pst1); pst2)
            have hlt : sz q_x < sz (p (p q_v1 (p q_x q_v2)) q_v1) := Nat.lt_trans (Nat.lt_trans (sz_lt_p_left q_x q_v2) (sz_lt_p_right q_v1 (p q_x q_v2))) (sz_lt_p_left (p q_v1 (p q_x q_v2)) q_v1)
            exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have qs3B := step_bound qs3
          have qs3N := step_no_first qs3
...
theorem source_holds (x v0 v1 v2 : CM) :
    x = (eval (eval (eval v0 (eval (eval v1 (eval x v2)) v1)) (eval (eval x v0) x)) v0) := by
  let H0 := eval x v2
  have e0a : x = x := by
    change x = x
    rfl
  ...
  let H2 := eval v0 (eval (eval v1 (eval x v2)) v1)
  have e2a : v0 = v0 := by
    change v0 = v0
    rfl
  have e2b : (eval (eval v1 (eval x v2)) v1) = (p H1 v1) := by
    change (eval H1 v1) = (p H1 v1)
    exact (eval_raw (nr0 x v0 v1 v2 H1 s1))
  have s2 : Step v0 (p H1 v1) H2 := by
    rw [← e2a, ← e2b]
    exact eval_step v0 (eval (eval v1 (eval x v2)) v1)
  let H3 := eval x v0
  have e3a : x = x := by
    change x = x
    rfl
