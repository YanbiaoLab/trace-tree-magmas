...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 : CM, Step q_x q_v1 q_H0 ∧ Step q_H0 q_v0 q_H1 ∧ a = q_v0 ∧ b = (p (p q_x q_x) (p q_v0 (p q_v0 q_H1))) ∧ o = q_x := by
  exact match h with
...
theorem code_no_pair_left (v k : CM) :
  ...
  | raw =>
    ...
    | raw =>
      ...
      change v = (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) at e1
      have cyc : v = (p (p q_x q_x) (p (p v k) (p (p v k) (p (p q_x q_v1) (p v k))))) := (let peq0 : (p v k) = q_v0 := e0; let peq1 : v = (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) := e1; let pst0 : q_v0 = (p v k) := Eq.symm (peq0); let pst1 : (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0))) = (p (p v k) (p q_v0 (p (p q_x q_v1) q_v0))) := congrArg (fun q => p q (p q_v0 (p (p q_x q_v1) q_v0))) (pst0); let pst2 : (p q_v0 (p (p q_x q_v1) q_v0)) = (p (p v k) (p (p q_x q_v1) q_v0)) := congrArg (fun q => p q (p (p q_x q_v1) q_v0)) (pst0); let pst3 : (p (p q_x q_v1) q_v0) = (p (p q_x q_v1) (p v k)) := congrArg (fun q => p (p q_x q_v1) q) (pst0); let pst4 : (p (p v k) (p (p q_x q_v1) q_v0)) = (p (p v k) (p (p q_x q_v1) (p v k))) := congrArg (fun q => p (p v k) q) (pst3); let pst5 : (p q_v0 (p (p q_x q_v1) q_v0)) = (p (p v k) (p (p q_x q_v1) (p v k))) := Eq.trans (pst2) (pst4); let pst6 : (p (p v k) (p q_v0 (p (p q_x q_v1) q_v0))) = (p (p v k) (p (p v k) (p (p q_x q_v1) (p v k)))) := congrArg (fun q => p (p v k) q) (pst5); let pst7 : (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0))) = (p (p v k) (p (p v k) (p (p q_x q_v1) (p v k)))) := Eq.trans (pst1) (pst6); let pst8 : (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) = (p (p q_x q_x) (p (p v k) (p (p v k) (p (p q_x q_v1) (p v k))))) := congrArg (fun q => p (p q_x q_x) q) (pst7); let pst9 : v = (p (p q_x q_x) (p (p v k) (p (p v k) (p (p q_x q_v1) (p v k))))) := Eq.trans (peq1) (pst8); pst9)
      have hlt : sz v < sz (p (p q_x q_x) (p (p v k) (p (p v k) (p (p q_x q_v1) (p v k))))) := Nat.lt_trans (Nat.lt_trans (sz_lt_p_left v k) (sz_lt_p_left (p v k) (p (p v k) (p (p q_x q_v1) (p v k))))) (sz_lt_p_right (p q_x q_x) (p (p v k) (p (p v k) (p (p q_x q_v1) (p v k)))))
 ...
 :
  ...
  | raw =>
    ...
    | raw =>
      ...
      change x = q_v0 at e0
      have e1 := congrArg (fun q => q) hb
      change x = (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) at e1
      have cyc : q_v0 = (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) := (let peq0 : x = q_v0 := e0; let peq1 : x = (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) := e1; let pst0 : q_v0 = x := Eq.symm (peq0); let pst1 : q_v0 = (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) := Eq.trans (pst0) (peq1); pst1)
      have hlt : sz q_v0 < sz (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) := Nat.lt_trans (sz_lt_p_left q_v0 (p q_v0 (p (p q_x q_v1) q_v0))) (sz_lt_p_right (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0))))
      exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
    | hit qs1h =>
      have e0 := congrArg (fun q => q) ha
      change x = q_v0 at e0
  ...
  | hit s1h =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        have p1 := hb
        change H1 = (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) at p1
        have z1 := congrArg sz p1
        have p2 := ho
        change o = q_x at p2
        have z2 := congrArg sz p2
        have hx : sz q_v0 < sz (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) := by
          have q := hcB.1
          have eu : sz v0 = sz q_v0 := congrArg sz (p0)
          have ev : sz H1 = sz (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) := congrArg sz (p1)
          have q1 : sz q_v0 < sz H1 := lt_of_eq_of_lt eu.symm q
          exact lt_of_lt_of_eq q1 ev
        have hy : sz (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) < sz q_v0 := by
          have q := s1hB.2
          have ev : sz H1 = sz (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) := congrArg sz (p1)
          have eu : sz v0 = sz q_v0 := congrArg sz (p0)
          have q1 : sz (p (p q_x q_x) (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0)))) < sz v0 := lt_of_eq_of_lt ev.symm q
  ...
  | raw =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        have cyc : q_x = (p q_x q_x) := (let peq0 : v0 = q_v0 := e0; let peq1 : v0 = (p q_x q_x) := e1; let peq3 : v0 = (p q_v0 (p (p q_x q_v1) q_v0)) := e3; let pst0 : q_v0 = v0 := Eq.symm (peq0); let pst1 : q_v0 = (p q_x q_x) := Eq.trans (pst0) (peq1); let pst2 : v0 = (p q_x q_x) := Eq.trans (peq0) (pst1); let pst3 : (p q_x q_x) = v0 := Eq.symm (pst2); let pst4 : (p q_x q_x) = (p q_v0 (p (p q_x q_v1) q_v0)) := Eq.trans (pst3) (peq3); let pst5 : (p q_v0 (p (p q_x q_v1) q_v0)) = (p (p q_x q_x) (p (p q_x q_v1) q_v0)) := congrArg (fun q => p q (p (p q_x q_v1) q_v0)) (pst1); let pst6 : (p (p q_x q_v1) q_v0) = (p (p q_x q_v1) (p q_x q_x)) := congrArg (fun q => p (p q_x q_v1) q) (pst1); let pst7 : (p (p q_x q_x) (p (p q_x q_v1) q_v0)) = (p (p q_x q_x) (p (p q_x q_v1) (p q_x q_x))) := congrArg (fun q => p (p q_x q_x) q) (pst6); let pst8 : (p q_v0 (p (p q_x q_v1) q_v0)) = (p (p q_x q_x) (p (p q_x q_v1) (p q_x q_x))) := Eq.trans (pst5) (pst7); let pst9 : (p q_x q_x) = (p (p q_x q_x) (p (p q_x q_v1) (p q_x q_x))) := Eq.trans (pst4) (pst8); let pst10 : q_x = (p q_x q_x) := congrArg (fun q => L q) (pst9); pst10)
    ...
    | raw =>
      ...
      | raw =>
        ...
        have badlt : sz v0 < sz H1 := by
          have structural : sz q_v0 < sz (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0))) := sz_lt_p_left q_v0 (p q_v0 (p (p q_x q_v1) q_v0))
          have large_eq : sz v0 = sz q_v0 := congrArg sz (p0)
          have small_eq : sz H1 = sz (p q_v0 (p q_v0 (p (p q_x q_v1) q_v0))) := congrArg sz (p2)
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval v0 (eval (eval x x) (eval v0 (eval v0 (eval (eval x v1) v0))))) := by
  let H0 := eval x v1
  have e0a : x = x := by
    change x = x
    rfl
