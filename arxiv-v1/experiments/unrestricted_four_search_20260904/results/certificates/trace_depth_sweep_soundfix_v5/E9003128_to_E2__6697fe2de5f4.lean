...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 q_H2 : CM, Step q_x q_v1 q_H0 ∧ Step q_v0 (p q_H0 q_x) q_H1 ∧ Step q_x q_v0 q_H2 ∧ a = (p q_H1 (p q_H2 q_x)) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
...
theorem code_no_pair_right (v k : CM) :
  ...
  | raw =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        change v = (p q_v0 (p (p q_x q_v1) q_x)) at e0
      ...
      | hit qs2h =>
        ...
        change v = (p q_v0 (p (p q_x q_v1) q_x)) at p0
  ...
  | hit qs0h =>
    ...
    | raw =>
      ...
      | raw =>
        have e0 := congrArg (fun q => (L q)) ha
        change v = (p q_v0 (p q_H0 q_x)) at e0
        have e1 := congrArg (fun q => (R q)) ha
        change k = (p (p q_x q_v0) q_x) at e1
        have e2 := congrArg (fun q => q) hb
        change k = q_v0 at e2
      ...
      | hit qs2h =>
        ...
        have qs2B := qs2B
        have p0 := congrArg (fun q => (L q)) (ha)
        change v = (p q_v0 (p q_H0 q_x)) at p0
        have z0 := congrArg sz p0
        have p1 := congrArg (fun q => (R q)) (ha)
        change k = (p q_H2 q_x) at p1
        have z1 := congrArg sz p1
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          change b = (p (p q_x q_v1) q_x) at e1
          ...
          exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have cyc : q_x = (p (p q_x q_v1) q_x) := (let peq1 : b = (p (p q_x q_v1) q_x) := congrArg (fun q => (R (L q))) (ha); let peq4 : b = q_x := ho; let pst0 : (p (p q_x q_v1) q_x) = b := Eq.symm (peq1); let pst1 : (p (p q_x q_v1) q_x) = q_x := Eq.trans (pst0) (peq4); let pst2 : q_x = (p (p q_x q_v1) q_x) := Eq.symm (pst1); pst2)
    ...
    | hit qs0h =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have cyc : q_x = (p q_H0 q_x) := (let peq1 : b = (p q_H0 q_x) := congrArg (fun q => (R (L q))) (ha); let peq4 : b = q_x := ho; let pst0 : (p q_H0 q_x) = b := Eq.symm (peq1); let pst1 : (p q_H0 q_x) = q_x := Eq.trans (pst0) (peq4); let pst2 : q_x = (p q_H0 q_x) := Eq.symm (pst1); pst2)
...
theorem nr2 (x v0 v1 H1 H2 : CM)
  ...
  | raw =>
    cases qs1 with
    | raw =>
      have he : v0 = (p (p H2 x) (p q_H0 q_x)) := (let peq0 : v0 = (p q_v0 (p q_H0 q_x)) := congrArg (fun q => (L q)) (ha); let peq2 : x = q_x := congrArg (fun q => (R (R q))) (ha); let peq3 : (p H2 x) = q_v0 := hb; let pst0 : (p H2 x) = (p H2 q_x) := congrArg (fun q => p H2 q) (peq2); let pst1 : (p H2 q_x) = (p H2 x) := Eq.symm (pst0); let pst2 : (p H2 q_x) = q_v0 := Eq.trans (pst1) (peq3); let pst3 : q_v0 = (p H2 q_x) := Eq.symm (pst2); let pst4 : (p q_v0 (p q_H0 q_x)) = (p (p H2 q_x) (p q_H0 q_x)) := congrArg (fun q => p q (p q_H0 q_x)) (pst3); let pst5 : v0 = (p (p H2 q_x) (p q_H0 q_x)) := Eq.trans (peq0) (pst4); let pst6 : (p H2 x) = (p H2 q_x) := congrArg (fun q => p H2 q) (peq2); let pst7 : (p (p H2 x) (p q_H0 q_x)) = (p (p H2 q_x) (p q_H0 q_x)) := congrArg (fun q => p q (p q_H0 q_x)) (pst6); let pst8 : (p (p H2 q_x) (p q_H0 q_x)) = (p (p H2 x) (p q_H0 q_x)) := Eq.symm (pst7); let pst9 : v0 = (p (p H2 x) (p q_H0 q_x)) := Eq.trans (pst5) (pst8); pst9)
      have hs := congrArg sz he
      simp only [getOut, L, R, U, sz] at s2B hs
      omega
    | hit qs1h =>
      have ein : q_v0 = (p H2 x) := (let peq2 : x = q_x := congrArg (fun q => (R (R q))) (ha); let peq3 : (p H2 x) = q_v0 := hb; let pst0 : (p H2 x) = (p H2 q_x) := congrArg (fun q => p H2 q) (peq2); let pst1 : (p H2 q_x) = (p H2 x) := Eq.symm (pst0); let pst2 : (p H2 q_x) = q_v0 := Eq.trans (pst1) (peq3); let pst3 : q_v0 = (p H2 q_x) := Eq.symm (pst2); let pst4 : (p H2 x) = (p H2 q_x) := congrArg (fun q => p H2 q) (peq2); let pst5 : (p H2 q_x) = (p H2 x) := Eq.symm (pst4); let pst6 : q_v0 = (p H2 x) := Eq.trans (pst3) (pst5); pst6)
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval (eval (eval v0 (eval (eval x v1) x)) (eval (eval x v0) x)) v0) := by
  let H0 := eval x v1
  have e0a : x = x := by
    change x = x
    rfl
  ...
  let H1 := eval v0 (eval (eval x v1) x)
  have e1a : v0 = v0 := by
    change v0 = v0
    rfl
  have e1b : (eval (eval x v1) x) = (p H0 x) := by
    change (eval H0 x) = (p H0 x)
    exact (eval_raw (nr0 x v0 v1 H0 s0))
  have s1 : Step v0 (p H0 x) H1 := by
    rw [← e1a, ← e1b]
    exact eval_step v0 (eval (eval x v1) x)
  let H2 := eval x v0
  have e2a : x = x := by
    change x = x
    rfl
