...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_v2 q_H0 q_H1 q_H2 q_H3 : CM, Step q_v2 q_v0 q_H0 ∧ Step q_v1 q_H0 q_H1 ∧ Step q_v0 q_H1 q_H2 ∧ Step q_x q_v0 q_H3 ∧ a = (p q_H2 (p q_H3 q_x)) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          change v = (p q_v0 (p q_v1 (p q_v2 q_v0))) at e0
        ...
        | hit qs3h =>
          ...
          change v = (p q_v0 (p q_v1 (p q_v2 q_v0))) at p0
      ...
      | hit qs2h =>
        ...
        | hit qs3h =>
          ...
          change o = q_x at p3
          have z3 := congrArg sz p3
          have badlt : sz q_v0 < sz (p q_v1 (p q_v2 q_v0)) := by
            have structural : sz (p q_H3 q_x) < sz (p q_v1 (p q_v2 (p q_H3 q_x))) := Nat.lt_trans (sz_lt_p_right q_v2 (p q_H3 q_x)) (sz_lt_p_right q_v1 (p q_v2 (p q_H3 q_x)))
            have large_eq : sz q_v0 = sz (p q_H3 q_x) := congrArg sz (Eq.trans (p2.symm) (p1))
            have small_eq : sz (p q_v1 (p q_v2 q_v0)) = sz (p q_v1 (p q_v2 (p q_H3 q_x))) := congrArg sz (congrArg (fun q => p q_v1 q) (congrArg (fun q => p q_v2 q) (Eq.trans (p2.symm) (p1))))
            exact lt_of_lt_of_eq (lt_of_eq_of_lt large_eq structural) small_eq.symm
  ...
  | hit qs0h =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          have e0 := congrArg (fun q => (L q)) ha
          change v = (p q_v0 (p q_v1 q_H0)) at e0
          have e1 := congrArg (fun q => (R q)) ha
          change k = (p (p q_x q_v0) q_x) at e1
          have e2 := congrArg (fun q => q) hb
          change k = q_v0 at e2
        ...
        | hit qs3h =>
          ...
          have qs3B := qs3B
          have p0 := congrArg (fun q => (L q)) (ha)
          change v = (p q_v0 (p q_v1 q_H0)) at p0
          have z0 := congrArg sz p0
          have p1 := congrArg (fun q => (R q)) (ha)
          change k = (p q_H3 q_x) at p1
          have z1 := congrArg sz p1
      ...
      | raw =>
        ...
        | raw =>
          ...
          | raw =>
            ...
            change b = (p q_v1 (p q_v2 q_v0)) at e1
            ...
            exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
          | hit qs3h =>
            have cyc : q_x = (p q_v1 (p q_v2 (p q_H3 q_x))) := (let peq0 : a = q_v0 := congrArg (fun q => (L (L q))) (ha); let peq1 : b = (p q_v1 (p q_v2 q_v0)) := congrArg (fun q => (R (L q))) (ha); let peq2 : a = (p q_H3 q_x) := congrArg (fun q => (R q)) (ha); let peq4 : b = q_x := ho; let pst0 : q_v0 = a := Eq.symm (peq0); let pst1 : q_v0 = (p q_H3 q_x) := Eq.trans (pst0) (peq2); let pst2 : (p q_v2 q_v0) = (p q_v2 (p q_H3 q_x)) := congrArg (fun q => p q_v2 q) (pst1); let pst3 : (p q_v1 (p q_v2 q_v0)) = (p q_v1 (p q_v2 (p q_H3 q_x))) := congrArg (fun q => p q_v1 q) (pst2); let pst4 : b = (p q_v1 (p q_v2 (p q_H3 q_x))) := Eq.trans (peq1) (pst3); let pst5 : (p q_v1 (p q_v2 (p q_H3 q_x))) = b := Eq.symm (pst4); let pst6 : (p q_v1 (p q_v2 (p q_H3 q_x))) = q_x := Eq.trans (pst5) (peq4); let pst7 : q_x = (p q_v1 (p q_v2 (p q_H3 q_x))) := Eq.symm (pst6); pst7)
            have hlt : sz q_x < sz (p q_v1 (p q_v2 (p q_H3 q_x))) := Nat.lt_trans (Nat.lt_trans (sz_lt_p_right q_H3 q_x) (sz_lt_p_right q_v2 (p q_H3 q_x))) (sz_lt_p_right q_v1 (p q_v2 (p q_H3 q_x)))
            exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have qs3B := step_bound qs3
          have qs3N := step_no_first qs3
          ...
          | raw =>
            ...
            have z3 := congrArg sz p3
            have badlt : sz q_v0 < sz (p q_v1 (p q_v2 q_v0)) := by
              have structural : sz q_v0 < sz (p q_v1 (p q_v2 q_v0)) := Nat.lt_trans (sz_lt_p_right q_v2 q_v0) (sz_lt_p_right q_v1 (p q_v2 q_v0))
              have large_eq : sz q_v0 = sz q_v0 := congrArg sz (rfl)
              have small_eq : sz (p q_v1 (p q_v2 q_v0)) = sz (p q_v1 (p q_v2 q_v0)) := congrArg sz (rfl)
              exact lt_of_lt_of_eq (lt_of_eq_of_lt large_eq structural) small_eq.symm
            exact (Nat.not_lt_of_ge (Nat.le_of_lt badlt) qs2hB.1).elim
          | hit qs3h =>
            have hcB := code_bounds hc
      ...
      | raw =>
        ...
        | raw =>
          ...
          | raw =>
            ...
            exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
          | hit qs3h =>
            have epa : (p (p a b) a) = (p (p (p q_H3 (p q_v1 q_H0)) (p q_v1 q_H0)) (p q_H3 (p q_v1 q_H0))) := Eq.trans (congrArg (fun q => p q a) (Eq.trans (congrArg (fun q => p q b) (Eq.trans (congrArg (fun q => (L (L q))) (ha)) (Eq.trans (Eq.trans (Eq.symm (congrArg (fun q => (L (L q))) (ha))) (congrArg (fun q => (R q)) (ha))) (congrArg (fun q => p q_H3 q) (Eq.symm (Eq.trans (Eq.symm (congrArg (fun q => (R (L q))) (ha))) (ho))))))) (congrArg (fun q => p (p q_H3 (p q_v1 q_H0)) q) (congrArg (fun q => (R (L q))) (ha))))) (congrArg (fun q => p (p (p q_H3 (p q_v1 q_H0)) (p q_v1 q_H0)) q) (Eq.trans (congrArg (fun q => (L (L q))) (ha)) (Eq.trans (Eq.trans (Eq.symm (congrArg (fun q => (L (L q))) (ha))) (congrArg (fun q => (R q)) (ha))) (congrArg (fun q => p q_H3 q) (Eq.symm (Eq.trans (Eq.symm (congrArg (fun q => (R (L q))) (ha))) (ho)))))))
            have epb : k = (p q_H3 (p q_v1 q_H0)) := Eq.trans (Eq.trans (hb) (Eq.trans (Eq.symm (congrArg (fun q => (L (L q))) (ha))) (congrArg (fun q => (R q)) (ha)))) (congrArg (fun q => p q_H3 q) (Eq.symm (Eq.trans (Eq.symm (congrArg (fun q => (R (L q))) (ha))) (ho))))
            apply code_no_pair_right (p (p q_H3 (p q_v1 q_H0)) (p q_v1 q_H0)) (p q_H3 (p q_v1 q_H0))
            exact ⟨_, by simpa only [epa, epb] using hc⟩
        | hit qs2h =>
          have qs3B := step_bound qs3
          have qs3N := step_no_first qs3
...
theorem source_holds (x v0 v1 v2 : CM) :
    x = (eval (eval (eval v0 (eval v1 (eval v2 v0))) (eval (eval x v0) x)) v0) := by
  let H0 := eval v2 v0
  have e0a : v2 = v2 := by
    change v2 = v2
    rfl
