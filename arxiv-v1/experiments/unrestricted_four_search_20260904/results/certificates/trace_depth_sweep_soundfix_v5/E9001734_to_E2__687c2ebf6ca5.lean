...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 q_H2 : CM, Step q_x q_v1 q_H0 ∧ Step q_v0 q_H0 q_H1 ∧ Step q_x q_v0 q_H2 ∧ a = (p q_H1 (p q_H2 q_x)) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
  ...
  | raw =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        change v = (p q_v0 (p q_x q_v1)) at e0
      ...
      | hit qs2h =>
        ...
        change v = (p q_v0 (p q_x q_v1)) at p0
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have cyc : q_x = (p q_x q_v1) := (let peq1 : b = (p q_x q_v1) := congrArg (fun q => (R (L q))) (ha); let peq4 : b = q_x := ho; let pst0 : (p q_x q_v1) = b := Eq.symm (peq1); let pst1 : (p q_x q_v1) = q_x := Eq.trans (pst0) (peq4); let pst2 : q_x = (p q_x q_v1) := Eq.symm (pst1); pst2)
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval (eval (eval v0 (eval x v1)) (eval (eval x v0) x)) v0) := by
  let H0 := eval x v1
  have e0a : x = x := by
    change x = x
    rfl
