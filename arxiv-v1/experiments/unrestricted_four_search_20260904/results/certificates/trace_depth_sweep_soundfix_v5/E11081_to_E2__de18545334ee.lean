...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 : CM, Step q_v0 q_v1 q_H0 ∧ Step q_x q_v0 q_H1 ∧ a = (p q_H0 (p q_H1 q_x)) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
  ...
  | raw =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
      | hit qs1h =>
        have epa : (p (p a b) a) = (p (p (p q_H1 q_x) q_x) (p q_H1 q_x)) := Eq.trans (congrArg (fun q => p q a) (Eq.trans (congrArg (fun q => p q b) (Eq.trans (congrArg (fun q => (L (L q))) (ha)) (Eq.trans (Eq.symm (congrArg (fun q => (L (L q))) (ha))) (congrArg (fun q => (R q)) (ha))))) (congrArg (fun q => p (p q_H1 q_x) q) (Eq.trans (congrArg (fun q => (R (L q))) (ha)) (Eq.trans (Eq.symm (congrArg (fun q => (R (L q))) (ha))) (ho)))))) (congrArg (fun q => p (p (p q_H1 q_x) q_x) q) (Eq.trans (congrArg (fun q => (L (L q))) (ha)) (Eq.trans (Eq.symm (congrArg (fun q => (L (L q))) (ha))) (congrArg (fun q => (R q)) (ha)))))
        have epb : k = (p q_H1 q_x) := Eq.trans (hb) (Eq.trans (Eq.symm (congrArg (fun q => (L (L q))) (ha))) (congrArg (fun q => (R q)) (ha)))
        apply code_no_pair_right (p (p q_H1 q_x) q_x) (p q_H1 q_x)
...
theorem nr1 (x v0 v1 H0 H1 : CM)
  ...
  | raw =>
    cases qs0 with
    | raw =>
      have he : v0 = (p (p H1 x) q_v1) := (let peq0 : v0 = (p q_v0 q_v1) := congrArg (fun q => (L q)) (ha); let peq2 : (p H1 x) = q_v0 := hb; let pst0 : q_v0 = (p H1 x) := Eq.symm (peq2); let pst1 : (p q_v0 q_v1) = (p (p H1 x) q_v1) := congrArg (fun q => p q q_v1) (pst0); let pst2 : v0 = (p (p H1 x) q_v1) := Eq.trans (peq0) (pst1); pst2)
      have hs := congrArg sz he
      simp only [getOut, L, R, U, sz] at s1B hs
      omega
    | hit qs0h =>
      have ein : q_v0 = (p H1 x) := (let peq2 : (p H1 x) = q_v0 := hb; let pst0 : q_v0 = (p H1 x) := Eq.symm (peq2); pst0)
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval (eval (eval v0 v1) (eval (eval x v0) x)) v0) := by
  let H0 := eval v0 v1
  have e0a : v0 = v0 := by
    change v0 = v0
    rfl
  ...
  have rawEq : (eval (eval H0 (eval H1 x)) v0) = (eval (p H0 (p H1 x)) v0) := by
    calc
      (eval (eval H0 (eval H1 x)) v0) = (eval (eval H0 (p H1 x)) v0) := congrArg (fun q => (eval (eval H0 q) v0)) (eval_raw (nr0 x v0 v1 H1 s1))
      _ = (eval (p H0 (p H1 x)) v0) := congrArg (fun q => (eval q v0)) (eval_raw (nr1 x v0 v1 H0 H1 s0 s1))
