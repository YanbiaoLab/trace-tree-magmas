...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 q_H2 : CM, Step q_v1 q_v0 q_H0 ∧ Step q_v0 q_H0 q_H1 ∧ Step q_x q_v0 q_H2 ∧ a = (p q_H1 (p q_H2 q_x)) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
  ...
  | raw =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        change v = (p q_v0 (p q_v1 q_v0)) at e0
      ...
      | hit qs2h =>
        ...
        change v = (p q_v0 (p q_v1 q_v0)) at p0
    ...
    | hit qs1h =>
      ...
      | hit qs2h =>
        ...
        have badlt : sz q_v0 < sz (p q_v1 q_v0) := by
          have structural : sz (p q_H2 q_x) < sz (p q_v1 (p q_H2 q_x)) := sz_lt_p_right q_v1 (p q_H2 q_x)
          have large_eq : sz q_v0 = sz (p q_H2 q_x) := congrArg sz (Eq.trans (p2.symm) (p1))
          have small_eq : sz (p q_v1 q_v0) = sz (p q_v1 (p q_H2 q_x)) := congrArg sz (congrArg (fun q => p q_v1 q) (Eq.trans (p2.symm) (p1)))
          exact lt_of_lt_of_eq (lt_of_eq_of_lt large_eq structural) small_eq.symm
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have cyc : q_x = (p q_v1 (p q_H2 q_x)) := (let peq0 : a = q_v0 := congrArg (fun q => (L (L q))) (ha); let peq1 : b = (p q_v1 q_v0) := congrArg (fun q => (R (L q))) (ha); let peq2 : a = (p q_H2 q_x) := congrArg (fun q => (R q)) (ha); let peq4 : b = q_x := ho; let pst0 : q_v0 = a := Eq.symm (peq0); let pst1 : q_v0 = (p q_H2 q_x) := Eq.trans (pst0) (peq2); let pst2 : (p q_v1 q_v0) = (p q_v1 (p q_H2 q_x)) := congrArg (fun q => p q_v1 q) (pst1); let pst3 : b = (p q_v1 (p q_H2 q_x)) := Eq.trans (peq1) (pst2); let pst4 : (p q_v1 (p q_H2 q_x)) = b := Eq.symm (pst3); let pst5 : (p q_v1 (p q_H2 q_x)) = q_x := Eq.trans (pst4) (peq4); let pst6 : q_x = (p q_v1 (p q_H2 q_x)) := Eq.symm (pst5); pst6)
          have hlt : sz q_x < sz (p q_v1 (p q_H2 q_x)) := Nat.lt_trans (sz_lt_p_right q_H2 q_x) (sz_lt_p_right q_v1 (p q_H2 q_x))
          exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
      | hit qs1h =>
        have qs2B := step_bound qs2
        have qs2N := step_no_first qs2
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval (eval (eval v0 (eval v1 v0)) (eval (eval x v0) x)) v0) := by
  let H0 := eval v1 v0
  have e0a : v1 = v1 := by
    change v1 = v1
    rfl
