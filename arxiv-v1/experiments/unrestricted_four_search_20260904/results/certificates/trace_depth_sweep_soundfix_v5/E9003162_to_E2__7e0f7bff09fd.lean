...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 q_H2 : CM, Step q_v0 q_v1 q_H0 ∧ Step q_v0 q_H0 q_H1 ∧ Step q_x q_v0 q_H2 ∧ a = (p q_H1 (p q_H2 q_x)) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
  ...
  | raw =>
    ...
    | raw =>
      ...
      | hit qs2h =>
        ...
        change v = (p q_v0 (p q_v0 q_v1)) at p0
    ...
    | hit qs1h =>
      ...
      | hit qs2h =>
        ...
        have badlt : sz q_v0 < sz (p q_v0 q_v1) := by
          have structural : sz (p q_H2 q_x) < sz (p (p q_H2 q_x) q_v1) := sz_lt_p_left (p q_H2 q_x) q_v1
          have large_eq : sz q_v0 = sz (p q_H2 q_x) := congrArg sz (Eq.trans (p2.symm) (p1))
          have small_eq : sz (p q_v0 q_v1) = sz (p (p q_H2 q_x) q_v1) := congrArg sz (congrArg (fun q => p q q_v1) (Eq.trans (p2.symm) (p1)))
          exact lt_of_lt_of_eq (lt_of_eq_of_lt large_eq structural) small_eq.symm
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have cyc : q_x = (p (p q_H2 q_x) q_v1) := (let peq0 : a = q_v0 := congrArg (fun q => (L (L q))) (ha); let peq1 : b = (p q_v0 q_v1) := congrArg (fun q => (R (L q))) (ha); let peq2 : a = (p q_H2 q_x) := congrArg (fun q => (R q)) (ha); let peq4 : b = q_x := ho; let pst0 : q_v0 = a := Eq.symm (peq0); let pst1 : q_v0 = (p q_H2 q_x) := Eq.trans (pst0) (peq2); let pst2 : (p q_v0 q_v1) = (p (p q_H2 q_x) q_v1) := congrArg (fun q => p q q_v1) (pst1); let pst3 : b = (p (p q_H2 q_x) q_v1) := Eq.trans (peq1) (pst2); let pst4 : (p (p q_H2 q_x) q_v1) = b := Eq.symm (pst3); let pst5 : (p (p q_H2 q_x) q_v1) = q_x := Eq.trans (pst4) (peq4); let pst6 : q_x = (p (p q_H2 q_x) q_v1) := Eq.symm (pst5); pst6)
          have hlt : sz q_x < sz (p (p q_H2 q_x) q_v1) := Nat.lt_trans (sz_lt_p_right q_H2 q_x) (sz_lt_p_left (p q_H2 q_x) q_v1)
          exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
      | hit qs1h =>
        have qs2B := step_bound qs2
        have qs2N := step_no_first qs2
    ...
    | hit qs0h =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have ena : q_x = q_H0 := (let peq1 : b = q_H0 := congrArg (fun q => (R (L q))) (ha); let peq4 : b = q_x := ho; let pst0 : q_H0 = b := Eq.symm (peq1); let pst1 : q_H0 = q_x := Eq.trans (pst0) (peq4); let pst2 : q_x = q_H0 := Eq.symm (pst1); pst2)
          have enb : q_v0 = q_v0 := (let peq0 : a = q_v0 := congrArg (fun q => (L (L q))) (ha); let peq2 : a = (p q_H2 q_x) := congrArg (fun q => (R q)) (ha); let pst0 : q_v0 = a := Eq.symm (peq0); let pst1 : q_v0 = (p q_H2 q_x) := Eq.trans (pst0) (peq2); let pst2 : (p q_H2 q_x) = q_v0 := Eq.symm (pst1); let pst3 : q_v0 = q_v0 := Eq.trans (pst1) (pst2); pst3)
          apply qs0N
          refine ⟨q_H2, ?_⟩
          simpa only [ena, enb] using qs2h
      | hit qs1h =>
        have qs2B := step_bound qs2
        have qs2N := step_no_first qs2
        cases qs2 with
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval (eval (eval v0 (eval v0 v1)) (eval (eval x v0) x)) v0) := by
  let H0 := eval v0 v1
  have e0a : v0 = v0 := by
    change v0 = v0
    rfl
  ...
  have rawEq : (eval (eval H1 (eval H2 x)) v0) = (eval (p H1 (p H2 x)) v0) := by
    calc
      (eval (eval H1 (eval H2 x)) v0) = (eval (eval H1 (p H2 x)) v0) := congrArg (fun q => (eval (eval H1 q) v0)) (eval_raw (nr0 x v0 v1 H2 s2))
      _ = (eval (p H1 (p H2 x)) v0) := congrArg (fun q => (eval q v0)) (eval_raw (nr1 x v0 v1 H1 H2 s1 s2))
