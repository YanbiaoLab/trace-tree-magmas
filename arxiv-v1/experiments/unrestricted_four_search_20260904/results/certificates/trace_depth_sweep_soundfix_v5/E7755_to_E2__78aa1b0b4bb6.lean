...
theorem nr0 (x v0 v1 : CM)
 :
  ...
  | raw =>
    ...
    have cyc : q_v0 = (p q_v0 (p (p q_v1 (p q_x q_x)) q_v0)) := (let peq0 : x = q_v0 := e0; let peq1 : x = (p q_v0 (p (p q_v1 (p q_x q_x)) q_v0)) := e1; let pst0 : q_v0 = x := Eq.symm (peq0); let pst1 : q_v0 = (p q_v0 (p (p q_v1 (p q_x q_x)) q_v0)) := Eq.trans (pst0) (peq1); pst1)
...
theorem nr1 (x v0 v1 : CM)
 :
  ...
  rintro ⟨o, hc⟩
  rcases code_shape hc with ⟨q_x, q_v0, q_v1, q_H0, qs0, ha, hb, ho⟩
  have he : q_H0 = q_v0 := (let peq1 : x = q_v0 := congrArg (fun q => (L q)) (hb); let peq2 : x = q_H0 := congrArg (fun q => (R q)) (hb); let pst0 : q_v0 = x := Eq.symm (peq1); let pst1 : q_v0 = q_H0 := Eq.trans (pst0) (peq2); let pst2 : q_H0 = q_v0 := Eq.symm (pst1); pst2)
...
theorem nr2 (x v0 v1 H0 : CM)
  ...
  cases s0 with
  | raw =>
    have he : q_H0 = q_v0 := (let peq0 : v0 = q_v0 := ha; let peq1 : (p v1 (p x x)) = q_v0 := congrArg (fun q => (L q)) (hb); let peq2 : v0 = q_H0 := congrArg (fun q => (R q)) (hb); let pst0 : q_v0 = (p v1 (p x x)) := Eq.symm (peq1); let pst1 : v0 = (p v1 (p x x)) := Eq.trans (peq0) (pst0); let pst2 : (p v1 (p x x)) = v0 := Eq.symm (pst1); let pst3 : (p v1 (p x x)) = q_H0 := Eq.trans (pst2) (peq2); let pst4 : q_H0 = (p v1 (p x x)) := Eq.symm (pst3); let pst5 : (p v1 (p x x)) = q_v0 := Eq.symm (pst0); let pst6 : q_H0 = q_v0 := Eq.trans (pst4) (pst5); pst6)
    exact step_ne_second (by simpa only [he] using qs0)
  | hit s0h =>
    have qs0B := step_bound qs0
    cases qs0 with
    | raw =>
      ...
      have hx : sz q_v0 < sz (p q_v0 (p (p q_v1 (p q_x q_x)) q_v0)) := by
        ...
        have ev : sz H0 = sz (p q_v0 (p (p q_v1 (p q_x q_x)) q_v0)) := congrArg sz (p1)
    ...
    | hit qs0h =>
      ...
      have hx : sz q_v0 < sz (p q_v0 q_H0) := by
        have q := hcB.1
        have eu : sz v0 = sz q_v0 := congrArg sz (p0)
        have ev : sz H0 = sz (p q_v0 q_H0) := congrArg sz (p1)
        have q1 : sz q_v0 < sz H0 := lt_of_eq_of_lt eu.symm q
        exact lt_of_lt_of_eq q1 ev
