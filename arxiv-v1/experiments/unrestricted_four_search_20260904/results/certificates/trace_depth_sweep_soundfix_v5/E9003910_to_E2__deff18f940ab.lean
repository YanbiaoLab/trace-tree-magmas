...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_v2 q_H0 q_H1 q_H2 q_H3 q_H4 : CM, Step q_x q_v1 q_H0 ∧ Step q_H0 q_v2 q_H1 ∧ Step q_H1 q_v0 q_H2 ∧ Step q_H2 q_x q_H3 ∧ Step (p q_x q_H3) q_v0 q_H4 ∧ a = q_v0 ∧ b = (p q_v0 q_H4) ∧ o = q_x := by
  exact match h with
      ...
      | raw =>
        ...
        | raw =>
          ...
          | raw =>
            ...
            change v = (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) at e1
            have cyc : v = (p (p v k) (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) (p v k))) := (let peq0 : (p v k) = q_v0 := e0; let peq1 : v = (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) := e1; let pst0 : q_v0 = (p v k) := Eq.symm (peq0); let pst1 : (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) = (p (p v k) (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) := congrArg (fun q => p q (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) (pst0); let pst2 : (p (p (p q_x q_v1) q_v2) q_v0) = (p (p (p q_x q_v1) q_v2) (p v k)) := congrArg (fun q => p (p (p q_x q_v1) q_v2) q) (pst0); let pst3 : (p (p (p (p q_x q_v1) q_v2) q_v0) q_x) = (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x) := congrArg (fun q => p q q_x) (pst2); let pst4 : (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) = (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) := congrArg (fun q => p q_x q) (pst3); let pst5 : (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0) = (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) q_v0) := congrArg (fun q => p q q_v0) (pst4); let pst6 : (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) q_v0) = (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) (p v k)) := congrArg (fun q => p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) q) (pst0); let pst7 : (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0) = (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) (p v k)) := Eq.trans (pst5) (pst6); let pst8 : (p (p v k) (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) = (p (p v k) (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) (p v k))) := congrArg (fun q => p (p v k) q) (pst7); let pst9 : (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) = (p (p v k) (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) (p v k))) := Eq.trans (pst1) (pst8); let pst10 : v = (p (p v k) (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) (p v k))) := Eq.trans (peq1) (pst9); pst10)
            have hlt : sz v < sz (p (p v k) (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) (p v k))) := Nat.lt_trans (sz_lt_p_left v k) (sz_lt_p_left (p v k) (p (p q_x (p (p (p (p q_x q_v1) q_v2) (p v k)) q_x)) (p v k)))
        ...
        | raw =>
          ...
          | raw =>
            ...
            | raw =>
              ...
              have p1 := hb
              change H3 = (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) at p1
              have z1 := congrArg sz p1
              have p2 := ho
              change o = q_x at p2
              have z2 := congrArg sz p2
              have hx : sz q_v0 < sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) := by
                have q := hcB.1
                have eu : sz x = sz q_v0 := congrArg sz (p0)
                have ev : sz H3 = sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) := congrArg sz (p1)
                have q1 : sz q_v0 < sz H3 := lt_of_eq_of_lt eu.symm q
                exact lt_of_lt_of_eq q1 ev
              have hy : sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) < sz q_v0 := by
                have q := s3hB.2
                have ev : sz H3 = sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) := congrArg sz (p1)
                have eu : sz x = sz q_v0 := congrArg sz (p0)
                have q1 : sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) < sz x := lt_of_eq_of_lt ev.symm q
        ...
        | raw =>
          ...
          | raw =>
            ...
            | raw =>
              ...
              have z0 := congrArg sz p0
              have p1 := hb
              change H4 = (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) at p1
              have z1 := congrArg sz p1
              have p2 := ho
              change o = q_x at p2
              have z2 := congrArg sz p2
              have hx : sz q_v0 < sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) := by
                ...
                have ev : sz H4 = sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) := congrArg sz (p1)
              ...
              have hy : sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) < sz q_v0 := by
                ...
                have ev : sz H4 = sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) := congrArg sz (p1)
                have eu : sz v0 = sz q_v0 := congrArg sz (p0)
                have q1 : sz (p q_v0 (p (p q_x (p (p (p (p q_x q_v1) q_v2) q_v0) q_x)) q_v0)) < sz v0 := lt_of_eq_of_lt ev.symm q
                exact lt_of_lt_of_eq q1 eu
              exact (Nat.not_lt_of_ge (Nat.le_of_lt hx) hy).elim
            | hit qs4h =>
              have hcB := code_bounds hc
...
theorem source_holds (x v0 v1 v2 : CM) :
    x = (eval v0 (eval v0 (eval (eval x (eval (eval (eval (eval x v1) v2) v0) x)) v0))) := by
  let H0 := eval x v1
  have e0a : x = x := by
    change x = x
    rfl
  ...
  let H2 := eval (eval (eval x v1) v2) v0
  have e2a : (eval (eval x v1) v2) = H1 := by
    change H1 = H1
    rfl
  have e2b : v0 = v0 := by
    change v0 = v0
  ...
  have s2 : Step H1 v0 H2 := by
    rw [← e2a, ← e2b]
    exact eval_step (eval (eval x v1) v2) v0
  let H3 := eval (eval (eval (eval x v1) v2) v0) x
  have e3a : (eval (eval (eval x v1) v2) v0) = H2 := by
    change H2 = H2
    rfl
  have e3b : x = x := by
    change x = x
  ...
  have s3 : Step H2 x H3 := by
    rw [← e3a, ← e3b]
    exact eval_step (eval (eval (eval x v1) v2) v0) x
  let H4 := eval (eval x (eval (eval (eval (eval x v1) v2) v0) x)) v0
  have e4a : (eval x (eval (eval (eval (eval x v1) v2) v0) x)) = (p x H3) := by
  ...
  have s4 : Step (p x H3) v0 H4 := by
    rw [← e4a, ← e4b]
    exact eval_step (eval x (eval (eval (eval (eval x v1) v2) v0) x)) v0
