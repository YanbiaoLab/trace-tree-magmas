...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_v2 q_H0 q_H1 q_H2 q_H3 : CM, Step q_v0 q_v1 q_H0 ∧ Step q_H0 q_v2 q_H1 ∧ Step q_v0 q_H1 q_H2 ∧ Step q_x q_v0 q_H3 ∧ a = (p q_H2 (p q_H3 q_x)) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          change v = (p q_v0 (p (p q_v0 q_v1) q_v2)) at e0
        ...
        | hit qs3h =>
          ...
          change v = (p q_v0 (p (p q_v0 q_v1) q_v2)) at p0
      ...
      | hit qs2h =>
        ...
        | hit qs3h =>
          ...
          change o = q_x at p3
          have z3 := congrArg sz p3
          have badlt : sz q_v0 < sz (p (p q_v0 q_v1) q_v2) := by
            have structural : sz (p q_H3 q_x) < sz (p (p (p q_H3 q_x) q_v1) q_v2) := Nat.lt_trans (sz_lt_p_left (p q_H3 q_x) q_v1) (sz_lt_p_left (p (p q_H3 q_x) q_v1) q_v2)
            have large_eq : sz q_v0 = sz (p q_H3 q_x) := congrArg sz (Eq.trans (p2.symm) (p1))
            have small_eq : sz (p (p q_v0 q_v1) q_v2) = sz (p (p (p q_H3 q_x) q_v1) q_v2) := congrArg sz (congrArg (fun q => p q q_v2) (congrArg (fun q => p q q_v1) (Eq.trans (p2.symm) (p1))))
            exact lt_of_lt_of_eq (lt_of_eq_of_lt large_eq structural) small_eq.symm
      ...
      | raw =>
        ...
        | raw =>
          ...
          | raw =>
            ...
            change b = (p (p q_v0 q_v1) q_v2) at e1
            ...
            exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
          | hit qs3h =>
            have cyc : q_x = (p (p (p q_H3 q_x) q_v1) q_v2) := (let peq0 : a = q_v0 := congrArg (fun q => (L (L q))) (ha); let peq1 : b = (p (p q_v0 q_v1) q_v2) := congrArg (fun q => (R (L q))) (ha); let peq2 : a = (p q_H3 q_x) := congrArg (fun q => (R q)) (ha); let peq4 : b = q_x := ho; let pst0 : q_v0 = a := Eq.symm (peq0); let pst1 : q_v0 = (p q_H3 q_x) := Eq.trans (pst0) (peq2); let pst2 : (p q_v0 q_v1) = (p (p q_H3 q_x) q_v1) := congrArg (fun q => p q q_v1) (pst1); let pst3 : (p (p q_v0 q_v1) q_v2) = (p (p (p q_H3 q_x) q_v1) q_v2) := congrArg (fun q => p q q_v2) (pst2); let pst4 : b = (p (p (p q_H3 q_x) q_v1) q_v2) := Eq.trans (peq1) (pst3); let pst5 : (p (p (p q_H3 q_x) q_v1) q_v2) = b := Eq.symm (pst4); let pst6 : (p (p (p q_H3 q_x) q_v1) q_v2) = q_x := Eq.trans (pst5) (peq4); let pst7 : q_x = (p (p (p q_H3 q_x) q_v1) q_v2) := Eq.symm (pst6); pst7)
            have hlt : sz q_x < sz (p (p (p q_H3 q_x) q_v1) q_v2) := Nat.lt_trans (Nat.lt_trans (sz_lt_p_right q_H3 q_x) (sz_lt_p_left (p q_H3 q_x) q_v1)) (sz_lt_p_left (p (p q_H3 q_x) q_v1) q_v2)
            exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have qs3B := step_bound qs3
          have qs3N := step_no_first qs3
          ...
          | raw =>
            ...
            have z3 := congrArg sz p3
            have badlt : sz q_v0 < sz (p (p q_v0 q_v1) q_v2) := by
              have structural : sz q_v0 < sz (p (p q_v0 q_v1) q_v2) := Nat.lt_trans (sz_lt_p_left q_v0 q_v1) (sz_lt_p_left (p q_v0 q_v1) q_v2)
              have large_eq : sz q_v0 = sz q_v0 := congrArg sz (rfl)
              have small_eq : sz (p (p q_v0 q_v1) q_v2) = sz (p (p q_v0 q_v1) q_v2) := congrArg sz (rfl)
              exact lt_of_lt_of_eq (lt_of_eq_of_lt large_eq structural) small_eq.symm
            exact (Nat.not_lt_of_ge (Nat.le_of_lt badlt) qs2hB.1).elim
          | hit qs3h =>
            have hcB := code_bounds hc
...
theorem source_holds (x v0 v1 v2 : CM) :
    x = (eval (eval (eval v0 (eval (eval v0 v1) v2)) (eval (eval x v0) x)) v0) := by
  let H0 := eval v0 v1
  have e0a : v0 = v0 := by
    change v0 = v0
    rfl
