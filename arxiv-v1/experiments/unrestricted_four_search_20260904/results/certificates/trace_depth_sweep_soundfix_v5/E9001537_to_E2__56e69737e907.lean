...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 q_H2 q_H3 : CM, Step q_x q_v1 q_H0 ∧ Step q_H0 q_v1 q_H1 ∧ Step q_x q_H1 q_H2 ∧ Step q_v0 (p q_H2 q_x) q_H3 ∧ a = (p q_H3 q_v0) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval (eval (eval v0 (eval (eval x (eval (eval x v1) v1)) x)) v0) v0) := by
  let H0 := eval x v1
  have e0a : x = x := by
    change x = x
    rfl
  ...
  have s1 : Step H0 v1 H1 := by
    rw [← e1a, ← e1b]
    exact eval_step (eval x v1) v1
  let H2 := eval x (eval (eval x v1) v1)
  have e2a : x = x := by
    change x = x
    rfl
  have e2b : (eval (eval x v1) v1) = H1 := by
    change H1 = H1
    rfl
  have s2 : Step x H1 H2 := by
    rw [← e2a, ← e2b]
    exact eval_step x (eval (eval x v1) v1)
  let H3 := eval v0 (eval (eval x (eval (eval x v1) v1)) x)
  have e3a : v0 = v0 := by
    change v0 = v0
    rfl
  have e3b : (eval (eval x (eval (eval x v1) v1)) x) = (p H2 x) := by
    change (eval H2 x) = (p H2 x)
    exact (eval_raw (nr0 x v0 v1 H2 s2))
  have s3 : Step v0 (p H2 x) H3 := by
    rw [← e3a, ← e3b]
    exact eval_step v0 (eval (eval x (eval (eval x v1) v1)) x)
