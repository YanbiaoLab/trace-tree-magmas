...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 : CM, Step q_x q_v1 q_H0 ∧ Step q_v0 (p q_H0 q_x) q_H1 ∧ a = (p q_H1 q_v0) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval (eval (eval v0 (eval (eval x v1) x)) v0) v0) := by
  let H0 := eval x v1
  have e0a : x = x := by
    change x = x
    rfl
  ...
  have rawEq : (eval (eval H1 v0) v0) = (eval (p H1 v0) v0) := congrArg (fun q => (eval q v0)) (eval_raw (nr1 x v0 v1 H1 s1))
