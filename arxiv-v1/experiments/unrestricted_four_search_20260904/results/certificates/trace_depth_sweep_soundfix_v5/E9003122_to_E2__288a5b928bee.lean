...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_v2 q_H0 q_H1 q_H2 q_H3 : CM, Step q_x q_v2 q_H0 ∧ Step q_v1 q_H0 q_H1 ∧ Step q_v0 q_H1 q_H2 ∧ Step q_x q_v0 q_H3 ∧ a = (p q_H2 (p q_H3 q_x)) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          change v = (p q_v0 (p q_v1 (p q_x q_v2))) at e0
        ...
        | hit qs3h =>
          ...
          change v = (p q_v0 (p q_v1 (p q_x q_v2))) at p0
      ...
      | raw =>
        ...
        | raw =>
          ...
          | raw =>
            ...
            change b = (p q_v1 (p q_x q_v2)) at e1
            ...
            exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
          | hit qs3h =>
            have cyc : q_x = (p q_v1 (p q_x q_v2)) := (let peq1 : b = (p q_v1 (p q_x q_v2)) := congrArg (fun q => (R (L q))) (ha); let peq4 : b = q_x := ho; let pst0 : (p q_v1 (p q_x q_v2)) = b := Eq.symm (peq1); let pst1 : (p q_v1 (p q_x q_v2)) = q_x := Eq.trans (pst0) (peq4); let pst2 : q_x = (p q_v1 (p q_x q_v2)) := Eq.symm (pst1); pst2)
            have hlt : sz q_x < sz (p q_v1 (p q_x q_v2)) := Nat.lt_trans (sz_lt_p_left q_x q_v2) (sz_lt_p_right q_v1 (p q_x q_v2))
            exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
        | hit qs2h =>
          have qs3B := step_bound qs3
          have qs3N := step_no_first qs3
...
theorem source_holds (x v0 v1 v2 : CM) :
    x = (eval (eval (eval v0 (eval v1 (eval x v2))) (eval (eval x v0) x)) v0) := by
  let H0 := eval x v2
  have e0a : x = x := by
    change x = x
    rfl
