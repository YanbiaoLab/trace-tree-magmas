...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_v2 q_H0 q_H1 q_H2 q_H3 q_H4 : CM, Step q_v1 q_v0 q_H0 ∧ Step q_H0 q_x q_H1 ∧ Step q_H1 q_v2 q_H2 ∧ Step q_x q_H2 q_H3 ∧ Step q_v0 (p q_H3 q_x) q_H4 ∧ a = (p q_H4 q_v0) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
...
theorem code_bounds {a b o : CM} (h : Code a b o) :
  ...
  ·
    cases s4 with
    | raw =>
      exact Nat.lt_trans (Nat.lt_trans (sz_lt_p_right q_H3 q_x) (sz_lt_p_right q_v0 (p q_H3 q_x))) (sz_lt_p_left (p q_v0 (p q_H3 q_x)) q_v0)
    | hit h4 =>
      exact Nat.lt_trans (Nat.lt_trans (sz_lt_p_right q_H3 q_x) (code_key_small h4)) (sz_lt_p_right q_H4 q_v0)
...
theorem code_unique {a b o q : CM} (h : Code a b o) (k : Code a b q) : o = q := by
  ...
  have er := step_second_unique hs4 rs4
  ...
  | raw =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        cases qs3 with
        | raw =>
          have he : q_H4 = q_v0 := (let peq0 : v = q_H4 := congrArg (fun q => (L q)) (ha); let peq2 : v = q_v0 := hb; let pst0 : q_H4 = v := Eq.symm (peq0); let pst1 : q_H4 = q_v0 := Eq.trans (pst0) (peq2); pst1)
          exact step_ne_first (by simpa only [he] using qs4)
        | hit qs3h =>
          have he : q_H4 = q_v0 := (let peq0 : v = q_H4 := congrArg (fun q => (L q)) (ha); let peq2 : v = q_v0 := hb; let pst0 : q_H4 = v := Eq.symm (peq0); let pst1 : q_H4 = q_v0 := Eq.trans (pst0) (peq2); pst1)
          exact step_ne_first (by simpa only [he] using qs4)
      | hit qs2h =>
...
theorem source_holds (x v0 v1 v2 : CM) :
    x = (eval (eval (eval v0 (eval (eval x (eval (eval (eval v1 v0) x) v2)) x)) v0) v0) := by
  let H0 := eval v1 v0
  have e0a : v1 = v1 := by
    change v1 = v1
    rfl
  ...
  let H2 := eval (eval (eval v1 v0) x) v2
  have e2a : (eval (eval v1 v0) x) = H1 := by
    change H1 = H1
    rfl
  have e2b : v2 = v2 := by
    change v2 = v2
  ...
  have s2 : Step H1 v2 H2 := by
    rw [← e2a, ← e2b]
    exact eval_step (eval (eval v1 v0) x) v2
  let H3 := eval x (eval (eval (eval v1 v0) x) v2)
  have e3a : x = x := by
    change x = x
    rfl
  have e3b : (eval (eval (eval v1 v0) x) v2) = H2 := by
    change H2 = H2
    rfl
  have s3 : Step x H2 H3 := by
    rw [← e3a, ← e3b]
    exact eval_step x (eval (eval (eval v1 v0) x) v2)
  let H4 := eval v0 (eval (eval x (eval (eval (eval v1 v0) x) v2)) x)
  have e4a : v0 = v0 := by
    change v0 = v0
    rfl
  have e4b : (eval (eval x (eval (eval (eval v1 v0) x) v2)) x) = (p H3 x) := by
    change (eval H3 x) = (p H3 x)
    exact (eval_raw (nr0 x v0 v1 v2 H3 s3))
  have s4 : Step v0 (p H3 x) H4 := by
    rw [← e4a, ← e4b]
    exact eval_step v0 (eval (eval x (eval (eval (eval v1 v0) x) v2)) x)
  change x = (eval (eval H4 v0) v0)
  have rawEq : (eval (eval H4 v0) v0) = (eval (p H4 v0) v0) := congrArg (fun q => (eval q v0)) (eval_raw (nr1 x v0 v1 v2 H4 s4))
