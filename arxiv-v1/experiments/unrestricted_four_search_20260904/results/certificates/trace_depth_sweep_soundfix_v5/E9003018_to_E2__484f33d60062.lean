...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 q_H2 : CM, Step q_v0 q_x q_H0 ∧ Step q_v1 q_H0 q_H1 ∧ Step q_H1 q_x q_H2 ∧ a = q_v0 ∧ b = (p (p q_x q_H2) (p (p q_v0 q_v0) q_v0)) ∧ o = q_x := by
  exact match h with
  ...
  | raw =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        change v = (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) at e1
        have cyc : v = (p (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) (p (p (p v k) (p v k)) (p v k))) := (let peq0 : (p v k) = q_v0 := e0; let peq1 : v = (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) := e1; let pst0 : q_v0 = (p v k) := Eq.symm (peq0); let pst1 : (p q_v0 q_x) = (p (p v k) q_x) := congrArg (fun q => p q q_x) (pst0); let pst2 : (p q_v1 (p q_v0 q_x)) = (p q_v1 (p (p v k) q_x)) := congrArg (fun q => p q_v1 q) (pst1); let pst3 : (p (p q_v1 (p q_v0 q_x)) q_x) = (p (p q_v1 (p (p v k) q_x)) q_x) := congrArg (fun q => p q q_x) (pst2); let pst4 : (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) = (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) := congrArg (fun q => p q_x q) (pst3); let pst5 : (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) = (p (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) := congrArg (fun q => p q (p (p q_v0 q_v0) q_v0)) (pst4); let pst6 : (p q_v0 q_v0) = (p (p v k) q_v0) := congrArg (fun q => p q q_v0) (pst0); let pst7 : (p (p v k) q_v0) = (p (p v k) (p v k)) := congrArg (fun q => p (p v k) q) (pst0); let pst8 : (p q_v0 q_v0) = (p (p v k) (p v k)) := Eq.trans (pst6) (pst7); let pst9 : (p (p q_v0 q_v0) q_v0) = (p (p (p v k) (p v k)) q_v0) := congrArg (fun q => p q q_v0) (pst8); let pst10 : (p (p (p v k) (p v k)) q_v0) = (p (p (p v k) (p v k)) (p v k)) := congrArg (fun q => p (p (p v k) (p v k)) q) (pst0); let pst11 : (p (p q_v0 q_v0) q_v0) = (p (p (p v k) (p v k)) (p v k)) := Eq.trans (pst9) (pst10); let pst12 : (p (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) = (p (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) (p (p (p v k) (p v k)) (p v k))) := congrArg (fun q => p (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) q) (pst11); let pst13 : (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) = (p (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) (p (p (p v k) (p v k)) (p v k))) := Eq.trans (pst5) (pst12); let pst14 : v = (p (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) (p (p (p v k) (p v k)) (p v k))) := Eq.trans (peq1) (pst13); pst14)
        have hlt : sz v < sz (p (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) (p (p (p v k) (p v k)) (p v k))) := Nat.lt_trans (Nat.lt_trans (Nat.lt_trans (Nat.lt_trans (Nat.lt_trans (sz_lt_p_left v k) (sz_lt_p_left (p v k) q_x)) (sz_lt_p_right q_v1 (p (p v k) q_x))) (sz_lt_p_left (p q_v1 (p (p v k) q_x)) q_x)) (sz_lt_p_right q_x (p (p q_v1 (p (p v k) q_x)) q_x))) (sz_lt_p_left (p q_x (p (p q_v1 (p (p v k) q_x)) q_x)) (p (p (p v k) (p v k)) (p v k)))
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          change H1 = (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) at e1
    ...
    | raw =>
      ...
      | raw =>
        ...
        | raw =>
          ...
          have p1 := hb
          change H2 = (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) at p1
          have z1 := congrArg sz p1
          have p2 := ho
          change o = q_x at p2
          have z2 := congrArg sz p2
          have hx : sz q_v0 < sz (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) := by
            have q := hcB.1
            have eu : sz x = sz q_v0 := congrArg sz (p0)
            have ev : sz H2 = sz (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) := congrArg sz (p1)
            have q1 : sz q_v0 < sz H2 := lt_of_eq_of_lt eu.symm q
            exact lt_of_lt_of_eq q1 ev
          have hy : sz (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) < sz q_v0 := by
            have q := s2hB.2
            have ev : sz H2 = sz (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) := congrArg sz (p1)
            have eu : sz x = sz q_v0 := congrArg sz (p0)
            have q1 : sz (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) < sz x := lt_of_eq_of_lt ev.symm q
  ...
  | raw =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        have e1 := congrArg (fun q => q) hb
        change v0 = (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) at e1
        have cyc : q_v0 = (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) := (let peq0 : v0 = q_v0 := e0; let peq1 : v0 = (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) := e1; let pst0 : q_v0 = v0 := Eq.symm (peq0); let pst1 : q_v0 = (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) := Eq.trans (pst0) (peq1); pst1)
        have hlt : sz q_v0 < sz (p (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0)) := Nat.lt_trans (Nat.lt_trans (Nat.lt_trans (Nat.lt_trans (sz_lt_p_left q_v0 q_x) (sz_lt_p_right q_v1 (p q_v0 q_x))) (sz_lt_p_left (p q_v1 (p q_v0 q_x)) q_x)) (sz_lt_p_right q_x (p (p q_v1 (p q_v0 q_x)) q_x))) (sz_lt_p_left (p q_x (p (p q_v1 (p q_v0 q_x)) q_x)) (p (p q_v0 q_v0) q_v0))
        exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
      | hit qs2h =>
        have e0 := congrArg (fun q => q) ha
        change v0 = q_v0 at e0
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval v0 (eval (eval x (eval (eval v1 (eval v0 x)) x)) (eval (eval v0 v0) v0))) := by
  let H0 := eval v0 x
  have e0a : v0 = v0 := by
    change v0 = v0
    rfl
  ...
  let H2 := eval (eval v1 (eval v0 x)) x
  ...
  have s2 : Step H1 x H2 := by
    rw [← e2a, ← e2b]
    exact eval_step (eval v1 (eval v0 x)) x
