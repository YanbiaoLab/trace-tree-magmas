...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_v2 q_H0 q_H1 q_H2 q_H3 q_H4 : CM, Step q_v1 q_x q_H0 ∧ Step q_v0 q_v2 q_H1 ∧ Step q_H0 q_H1 q_H2 ∧ Step q_x q_H2 q_H3 ∧ Step q_v0 (p q_H3 q_x) q_H4 ∧ a = (p q_H4 q_v0) ∧ b = q_v0 ∧ o = q_x := by
  exact match h with
...
theorem source_holds (x v0 v1 v2 : CM) :
    x = (eval (eval (eval v0 (eval (eval x (eval (eval v1 x) (eval v0 v2))) x)) v0) v0) := by
  let H0 := eval v1 x
  have e0a : v1 = v1 := by
    change v1 = v1
    rfl
  ...
  let H2 := eval (eval v1 x) (eval v0 v2)
  have e2a : (eval v1 x) = H0 := by
    change H0 = H0
    rfl
  ...
  have s2 : Step H0 H1 H2 := by
    rw [← e2a, ← e2b]
    exact eval_step (eval v1 x) (eval v0 v2)
  let H3 := eval x (eval (eval v1 x) (eval v0 v2))
  have e3a : x = x := by
    change x = x
    rfl
  have e3b : (eval (eval v1 x) (eval v0 v2)) = H2 := by
    change H2 = H2
    rfl
  have s3 : Step x H2 H3 := by
    rw [← e3a, ← e3b]
    exact eval_step x (eval (eval v1 x) (eval v0 v2))
  let H4 := eval v0 (eval (eval x (eval (eval v1 x) (eval v0 v2))) x)
  have e4a : v0 = v0 := by
    change v0 = v0
    rfl
  have e4b : (eval (eval x (eval (eval v1 x) (eval v0 v2))) x) = (p H3 x) := by
    change (eval H3 x) = (p H3 x)
    exact (eval_raw (nr0 x v0 v1 v2 H3 s3))
  have s4 : Step v0 (p H3 x) H4 := by
    rw [← e4a, ← e4b]
    exact eval_step v0 (eval (eval x (eval (eval v1 x) (eval v0 v2))) x)
