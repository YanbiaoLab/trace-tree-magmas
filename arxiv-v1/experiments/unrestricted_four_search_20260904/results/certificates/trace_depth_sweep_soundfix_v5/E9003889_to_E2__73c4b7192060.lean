...
theorem code_shape {a b o : CM} (h : Code a b o) :
    ∃ q_x q_v0 q_v1 q_H0 q_H1 : CM, Step q_x q_v1 q_H0 ∧ Step (p q_H0 (p q_x q_x)) q_v0 q_H1 ∧ a = q_v0 ∧ b = (p q_v0 q_H1) ∧ o = q_x := by
  exact match h with
...
theorem code_no_pair_left (v k : CM) :
  ...
  | raw =>
    ...
    | raw =>
      ...
      change v = (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) at e1
      have cyc : v = (p (p v k) (p (p (p q_x q_v1) (p q_x q_x)) (p v k))) := (let peq0 : (p v k) = q_v0 := e0; let peq1 : v = (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) := e1; let pst0 : q_v0 = (p v k) := Eq.symm (peq0); let pst1 : (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) = (p (p v k) (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) := congrArg (fun q => p q (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) (pst0); let pst2 : (p (p (p q_x q_v1) (p q_x q_x)) q_v0) = (p (p (p q_x q_v1) (p q_x q_x)) (p v k)) := congrArg (fun q => p (p (p q_x q_v1) (p q_x q_x)) q) (pst0); let pst3 : (p (p v k) (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) = (p (p v k) (p (p (p q_x q_v1) (p q_x q_x)) (p v k))) := congrArg (fun q => p (p v k) q) (pst2); let pst4 : (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) = (p (p v k) (p (p (p q_x q_v1) (p q_x q_x)) (p v k))) := Eq.trans (pst1) (pst3); let pst5 : v = (p (p v k) (p (p (p q_x q_v1) (p q_x q_x)) (p v k))) := Eq.trans (peq1) (pst4); pst5)
      have hlt : sz v < sz (p (p v k) (p (p (p q_x q_v1) (p q_x q_x)) (p v k))) := Nat.lt_trans (sz_lt_p_left v k) (sz_lt_p_left (p v k) (p (p (p q_x q_v1) (p q_x q_x)) (p v k)))
 ...
 :
  ...
  | raw =>
    ...
    | raw =>
      ...
      change x = q_v0 at e0
      have e1 := congrArg (fun q => q) hb
      change x = (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) at e1
      have cyc : q_v0 = (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) := (let peq0 : x = q_v0 := e0; let peq1 : x = (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) := e1; let pst0 : q_v0 = x := Eq.symm (peq0); let pst1 : q_v0 = (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) := Eq.trans (pst0) (peq1); pst1)
      have hlt : sz q_v0 < sz (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) := sz_lt_p_left q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)
      exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
    | hit qs1h =>
      have e0 := congrArg (fun q => q) ha
      change x = q_v0 at e0
  ...
  | raw =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        change x = (p (p (p q_x q_v1) (p q_x q_x)) q_v0) at e2
        have cyc : x = (p x v1) := (let peq0 : (p x v1) = q_v0 := e0; let peq1 : x = q_v0 := e1; let pst0 : q_v0 = (p x v1) := Eq.symm (peq0); let pst1 : x = (p x v1) := Eq.trans (peq1) (pst0); pst1)
    ...
    | hit qs0h =>
      ...
      | raw =>
        ...
        change x = q_v0 at e1
        have e2 := congrArg (fun q => (R q)) hb
        change x = (p (p q_H0 (p q_x q_x)) q_v0) at e2
        have cyc : x = (p x v1) := (let peq0 : (p x v1) = q_v0 := e0; let peq1 : x = q_v0 := e1; let pst0 : q_v0 = (p x v1) := Eq.symm (peq0); let pst1 : x = (p x v1) := Eq.trans (peq1) (pst0); pst1)
        have hlt : sz x < sz (p x v1) := sz_lt_p_left x v1
        exact (Nat.ne_of_lt hlt) (congrArg sz cyc)
      | hit qs1h =>
  ...
  | hit s1h =>
    ...
    | raw =>
      ...
      | raw =>
        ...
        have p1 := hb
        change H1 = (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) at p1
        have z1 := congrArg sz p1
        have p2 := ho
        change o = q_x at p2
        have z2 := congrArg sz p2
        have hx : sz q_v0 < sz (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) := by
          have q := hcB.1
          have eu : sz v0 = sz q_v0 := congrArg sz (p0)
          have ev : sz H1 = sz (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) := congrArg sz (p1)
          have q1 : sz q_v0 < sz H1 := lt_of_eq_of_lt eu.symm q
          exact lt_of_lt_of_eq q1 ev
        have hy : sz (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) < sz q_v0 := by
          have q := s1hB.2
          have ev : sz H1 = sz (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) := congrArg sz (p1)
          have eu : sz v0 = sz q_v0 := congrArg sz (p0)
          have q1 : sz (p q_v0 (p (p (p q_x q_v1) (p q_x q_x)) q_v0)) < sz v0 := lt_of_eq_of_lt ev.symm q
...
theorem source_holds (x v0 v1 : CM) :
    x = (eval v0 (eval v0 (eval (eval (eval x v1) (eval x x)) v0))) := by
  let H0 := eval x v1
  have e0a : x = x := by
    change x = x
    rfl
  ...
  let H1 := eval (eval (eval x v1) (eval x x)) v0
  have e1a : (eval (eval x v1) (eval x x)) = (p H0 (p x x)) := by
  ...
  have s1 : Step (p H0 (p x x)) v0 H1 := by
    rw [← e1a, ← e1b]
    exact eval_step (eval (eval x v1) (eval x x)) v0
