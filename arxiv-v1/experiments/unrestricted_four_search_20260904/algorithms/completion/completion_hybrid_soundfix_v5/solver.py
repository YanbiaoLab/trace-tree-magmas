...
def _v13_recompile_indexed_dag(lean, rule_arities, node_count):
    ...
    for original in lean.splitlines():
        ...
        if source_active and original.startswith('def Carrier'):
            lines.append('  all_goals first | rfl | (simp_all [sz] <;> omega) | omega | grind')
        ...
        if split is not None:
            ...
            lines.append('  | rfl')
            lines.append('  | (simp_all [sz] <;> omega)')
            lines.append('  | omega')
            lines.append('  | skip')
            continue
...
def _v13_strict_completion_candidate(source, target, index=-1, operational=False, priority=False, residual_policy=0, strict_semantic_gate=False, compound_only=False):
    ...
    indexed_ok = (indexed is not None and
                  len(indexed.encode('utf-8')) <= MAX_FALSE_CERTIFICATE_BYTES)
    # Occurs-cycle leaves need the regular tableau's explicit cycle proof for
    # small systems.  For larger completed systems, the regular case product
    # is prohibitively expensive, while the indexed compiler's constructor
    # elimination is substantially faster.  This is a proof-compilation
    # policy only: both candidates encode the identical completed model.
    use_indexed = indexed_ok and (
        regular_cert['closures'].get('occurs', 0) == 0
        or regular_cert['rules'] >= 5
    )
    ...
    code = indexed if use_indexed else (regular if regular_ok else None)
    fallbacks = (
        (regular,) if use_indexed and regular_ok
        else (indexed,) if indexed_ok and regular_ok
        else ()
    )
...
def _paper_v13_strict_search(source, target, seconds):
    ...
    jobs = [
        job for job in jobs
        if job[3].get("subprocedure") == "strict_completion"
    ]
...
def main():
    ...
    metadata = {
        ...
        "search_program": "completion_only",
        "profile": "completion_hybrid_soundfix_v5",
        "configured_search_seconds": SEARCH_BUDGET_SECONDS,
