#!/usr/bin/env python3
# Strict independent ablation executable: trace_only.
# Enabled subprocedure: trace.  All other v13 jobs are filtered.
...
def _v13_check_deadline():
    """Cooperatively stop one trace job at its scheduler deadline."""
    if time.monotonic() >= _v13_DEADLINE:
        raise TimeoutError("trace job deadline")
...
def _v13_materialize_equality_context(eqs):
    ...
    for index, (left, right, proof) in enumerate(eqs):
        ...
        name = f'peq{index}'
        # Pin the exact symbolic endpoints.  An unannotated local definition
        # can still inherit expected-type information through later dependent
        # compositions, causing Lean to elaborate a valid projection at a
        # rewritten endpoint.
        bindings.append(
            f'let {name} : {_v13_pterm(left)} = {_v13_pterm(right)} := {lean};'
        )
...
def _v13_close_materialized_equality(bindings, proof):
    ...
    def compile_node(current):
        ...
        steps.append(
            f'let {name} : {_v13_pterm(current.left)} = '
            f'{_v13_pterm(current.right)} := {expression};'
        )
    ...
    needed_bindings = [
        ...
        if binding.split(None, 2)[1] in used_leaves
    ]
    return '(' + ' '.join(needed_bindings + steps + [result]) + ')'
...
def _v13_infeasible(rows):
    ...
    for v in vs:
        ...
        for pi, (p, bp) in enumerate(pos):
            if pi & 15 == 0:
                _v13_check_deadline()
            for ni, (n, bn) in enumerate(neg):
                if ni & 255 == 0:
                    _v13_check_deadline()
                ap, an = (p[v], -n[v])
                keys = (set(p) | set(n)) - {v}
...
def _v13_trace_model(source, target, skip=0, tableau_depth=0):
    ...
    for mask in masks:
        ...
        # A size inequality can be unconditional even when the smaller term is
        # not literally a subtree (for example, two repeated variables on both
        # sides plus extra nonnegative summands on the larger side).  Detect
        # this with the same exact linear-arithmetic checker used below, so the
        # Lean certificate can avoid an unnecessary recursive bound proof.
        unconditional_bounds = True
        for small, large in size_pairs:
            violation = [_v13_size_row(ix[large], ix[small], 0)]
            names = {v for row, _constant in violation for v in row}
            violation += [({v: -1}, 0) for v in names]
            if not _v13_infeasible(violation):
                unconditional_bounds = False
                break
        ...
        if recursive:
            ...
            if mproofs and (not structural_bounds) and (not unconditional_bounds):
                mutual_bound = True
                ...
                if unconditional_bounds:
                    shape_fields = ', '.join(qnames + [f's{i}' for i in range(len(steps))] + ['ha', 'hb', 'ho'])
                    bound_body = f"  rcases code_shape h with ⟨{shape_fields}⟩\n  subst a\n  subst b\n  subst o\n  simp only [sz] <;> omega"
                elif structural_bounds:
                    qstruct = []
                    for small, large in size_pairs:
                    ...
                    bound_body = f'  rcases code_shape h with ⟨{shape_fields}⟩\n  subst a\n  subst b\n  subst x\n{bound_cases}'
                if unconditional_bounds or structural_bounds:
        ...
        def split_steps(k, hits, env, aa, bb, indent, expanded=(), extra_eqs=(), stepfacts=(), negfacts=(), goal_out='o', negoutfacts=()):
            ...
            if k == len(allsteps):
                ...
                for hp, ia, ib, io in pair_candidates:
                    ia, ib = (_v13_replace_vars(ia, env), _v13_replace_vars(ib, env))
                    # Prefer the actual dependent witness whenever its indices
                    # already have the forbidden pair shape.  Normalizing a
                    # second time can choose an equivalent but non-definitional
                    # index after ``cases Step``, making an unnecessary
                    # transport fail even though ``hp`` itself is exact.
                    if isinstance(ia, tuple):
                        for coord, theorem, enabled in ((0, 'code_no_pair_left', code_no_pair_left), (1, 'code_no_pair_right', code_no_pair_right)):
                            if enabled and ib == ia[coord]:
                                return '\n'.join([
                                    pad + f'apply {theorem} {_v13_pterm(ia[0])} {_v13_pterm(ia[1])}',
                                    pad + f'exact ⟨_, {hp}⟩',
                                ])
                    compact_env = _v13_proof_unifier(ceqs) or {}
                    nia, pia = _v13_normalization_proof(ia, compact_env)
                    nib, pib = _v13_normalization_proof(ib, compact_env)
                    # Large inlined equality expressions let Lean's expected
                    # type flow into repeated projections.  Materialize only
                    # those large DAGs; compact transports are both stable and
                    # much cheaper to synthesize under the search deadline.
                    if max(len(pia), len(pib)) > 4096:
                        materialized, typed_ceqs = _v13_materialize_equality_context(ceqs)
                        proof_env = _v13_proof_unifier(typed_ceqs) or {}
                        nia, pa = _v13_normalization_proof_typed(ia, proof_env)
                        nib, pb = _v13_normalization_proof_typed(ib, proof_env)
                        pia = ('rfl' if pa.left == pa.right else
                               _v13_close_materialized_equality(materialized, pa))
                        pib = ('rfl' if pb.left == pb.right else
                               _v13_close_materialized_equality(materialized, pb))
                ...
                for h, ia, ib, io in hits:
                    ...
                    for n, (small, large) in enumerate(size_pairs):
                        ...
                        if path is None:
                            continue
                        direct = lines + [pad + f'have badlt : sz {_v13_pterm(tri[large])} < sz {_v13_pterm(tri[small])} := by', pad + f'  have structural : sz {_v13_pterm(nl)} < sz {_v13_pterm(ns)} := {_v13_lt_proof(ns, path)}', pad + f'  have large_eq : sz {_v13_pterm(tri[large])} = sz {_v13_pterm(nl)} := congrArg sz ({pl})', pad + f'  have small_eq : sz {_v13_pterm(tri[small])} = sz {_v13_pterm(ns)} := congrArg sz ({ps})', pad + '  exact lt_of_lt_of_eq (lt_of_eq_of_lt large_eq structural) small_eq.symm', pad + f"exact (Nat.not_lt_of_ge (Nat.le_of_lt badlt) {bproj(h + 'B', n)}).elim"]
                        break
            ...
            hit_env = env
            hit_output = _v13_replace_vars(hname, env)
            if isinstance(hit_output, str) and proof.startswith('u'):
                # Expanded-tableau binders are introduced by a dependent
                # ``rcases code_shape`` and may be cleared by ``cases Step``.
                # Top-level outputs are stable and retaining their original
                # names keeps the proof search state smaller.
                kept_output = f'{proof}out'
                prefix += pad + f'let {kept_output} := {hit_output}\n'
                hit_env = env.copy()
                hit_env[hname] = kept_output
            hitcode = split_steps(k + 1, hits + [(hit_name, sa, sb, hname)], hit_env, aa, bb, indent + 1, expanded, extra_eqs, nextfacts, nextneg, goal_out, nextout)
        ...
        for (a, b), i in raw_index.items():
            ...
            if ntbody is None:
                ...
                # Abandon this mask, but keep enumerating later trace models.
                bad_target_certificate = True
                break
            lean += f'theorem nt{i} : ¬ ∃ o, Code {_v13_cm_lean(a)} {_v13_cm_lean(b)} o := by\n  rintro ⟨o, hc⟩\n{ntbody}\n'
        if bad_target_certificate:
            continue
...
def _v13_strict_completion_candidate(source, target, index=-1, operational=False, priority=False, residual_policy=0, strict_semantic_gate=False, compound_only=False):
    ...
    indexed = None
    if prefer_indexed and regular_model:
        try:
            ...
            indexed = None
    regular_ok = regular_bytes <= 20000
    indexed_ok = indexed is not None and len(indexed.encode('utf-8')) <= 20000
    if prefer_indexed and indexed_ok:
        code = indexed if indexed_ok else (regular if regular_ok else None)
        fallbacks = (regular,) if indexed_ok and regular_ok else ()
    else:
        code = regular if regular_ok else (indexed if indexed_ok else None)
        fallbacks = (indexed,) if regular_ok and indexed_ok else ()
    ...
    def finalize(candidate):
        if candidate is None or len(candidate.encode('utf-8')) > 20000:
            return None
        try:
...
def _paper_v13_strict_search(source, target, seconds):
    ...
    # Depth-focused trace strategy: each depth is a full candidate family, and
    # receives substantially more time than in the competition schedule.
    trace_shapes = [(0, depth) for depth in range(6)]
    ...
    # Strict ablation: filter before any search job executes.
    jobs = [
        job for job in jobs
        if job[3].get("subprocedure") == "trace"
    ]
...
def main():
    ...
    metadata = {
        ...
        "search_program": "trace_only",
        "profile": "trace_depth_sweep_soundfix_v4",
        "configured_search_seconds": SEARCH_BUDGET_SECONDS,
