#!/usr/bin/env python3
# Strict independent ablation executable: guarded_only.
# Enabled subprocedure: guarded_decoder.  All other v13 jobs are filtered.
...
def _v13_guarded_program_model(source, target):
    ...
    # All seven choices are forced by the checked Lean proof contract.  Trying
    # the unique admissible decoder first is coverage-equivalent to enumerating
    # 24,000 programs and rejecting 23,999 of them after finite testing.
    program = ('inner', 'ka', 'x', 'e', 'dec', 'e', 'pab')
    ...
    for opposite in (False, True):
        ...
        if not _v13_guarded_source_closed(source, opposite):
            continue
        if not _v13_guarded_holds(source, program, shallow, opposite):
            continue
...
def _paper_v13_strict_search(source, target, seconds):
    ...
    jobs = [
        job for job in jobs
        if job[3].get("subprocedure") == "guarded_decoder"
    ]
